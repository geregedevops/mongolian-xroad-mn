...
- **xroad_subsystems table on gerege backend** must mirror every Service-clients grant. Granting in UI alone doesn't make the backend trust the X-Road-Client header — the backend looks the subsystem up in this table to resolve to a `relying_parties` row. See `docs/onboarding-new-member-ss.md` for the SQL.
